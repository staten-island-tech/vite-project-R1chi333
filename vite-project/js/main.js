...
DOMSelectors.el.insertAdjacentHTML(
  "afterend",
  `<p id="keybindsign">keybinds: ${keybindOne} ${keybindTwo} ${keybindThree} ${keybindFour}</p>`
);
...
function opensettings() {
  DOMSelectors.el.insertAdjacentHTML(
    "afterend",
    `       <div id="backarrowthing">🢀</div>   <div class= "pinksetting"  id="settingsmenuu">
    ...
    .forEach((playableSongs) => {
      ...
      theSongCard.addEventListener("click", function () {
        ...
        themvthatsplaying.onended = function () {
          ...
          if (misscount === 0) {
            ...
            DOMSelectors.el.insertAdjacentHTML(
              "afterend",
              `<p id="=perfectcombo">PERFECT COMBO</p>`
            );
            setTimeout(() => {
      ...
      theSongCard.addEventListener("mouseover", function () {
        if (artOn === true) {
          DOMSelectors.el.insertAdjacentHTML(
            "afterend",
            `<div class="changeOpacityQuicker" id="albumcover"><img id="albumcoverart" src="${playableSongs.art}" alt=""></div>`
          );
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.key === keybindTwo) {
    ...
    noteline = Array.from(document.querySelectorAll(".note2")).pop();
  ...
  if (event.key === keybindThree) {
    ...
    noteline = Array.from(document.querySelectorAll(".note3")).pop();
    place = noteline.getBoundingClientRect();
    console.log(place.top);
    scoring();
    return place;
  ...
  if (event.key === keybindFour) {
    ...
    noteline = Array.from(document.querySelectorAll(".note4")).pop();
    place = noteline.getBoundingClientRect();
    console.log(place.top);
    scoring();
    return place;
...
function scoring() {
  if (
    (place.top >= 577 && place.top <= 654) ||
    (place.top >= 840 && place.top < 900)
  ) {
    noteline.remove();
    DOMSelectors.el.insertAdjacentHTML(
      "afterend",
  ...
  if (
    (place.top > 654 && place.top <= 681) ||
    (place.top > 829 && place.top < 840)
  ) {
    noteline.remove();
    DOMSelectors.el.insertAdjacentHTML(
      "afterend",
  ...
  if (
    (place.top > 681 && place.top <= 710) ||
    (place.top > 823 && place.top <= 839)
  ) {
    noteline.remove();
    DOMSelectors.el.insertAdjacentHTML(
      "afterend",
  ...
  if (place.top > 710 && place.top < 823) {
    noteline.remove();
    DOMSelectors.el.insertAdjacentHTML(
      "afterend",
      `    <div id="scoreannc">
